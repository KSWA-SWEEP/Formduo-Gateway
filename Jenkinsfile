...
pipeline {
  agent any
    ...
    environment {
        dockerHubRegistry = 'qkdrmsgh73/service-gateway'
    ...
    stages {
        ...
        stage('Checkout Application Git Branch') {
            ...
            post {
                    failure {
                      slackSend (channel: '#jenkins', color: '#FFFF00', message: "Repository Clone Failure !: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
        ...
        stage('Maven Jar Build') {
                ...
                post {
                        failure {
                          slackSend (channel: '#jenkins', color: '#FFFF00', message: "Maven Jar Build Failure !: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
        ...
        stage('Docker Image Build') {
                ...
                post {
                        failure {
                          slackSend (channel: '#jenkins', color: '#FFFF00', message: "Docker Image Build Failure !: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
        ...
        stage('Docker Image Push') {
                ...
                post {
                        failure {
                          slackSend (channel: '#jenkins', color: '#FFFF00', message: "Docker Image Push Failure !: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
        ...
        stage('K8S Manifest Update') {
                ...
                post {
                        failure {
                          slackSend (channel: '#jenkins', color: '#FFFF00', message: "K8S Manifest Update Failure !: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
